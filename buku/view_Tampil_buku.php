        ...
        <div class="grid" id="grid">
            ...
            <div class="card-book" data-title="<?= htmlspecialchars($b['judul_buku']) ?>" data-pub="<?= htmlspecialchars($b['penerbit'] ?? '') ?>">
                ...
                <div class="card-text-wrapper">
                    ...
                    <div class="info-line" style="margin-top: auto;"><i class="bi bi-calendar"></i> <?= htmlspecialchars($b['tahun_terbit'] ?? '—') ?></div>
                    <div class="btn-group">
...
<body>
    ...
    <div class="detail-container">
        <div class="detail-header">
            <a href="view_Tampil_buku.php" class="btn-back">
        ...
        <div class="detail-content">
            <!-- Book Cover -->
            <div class="book-cover">
                <img src="<?= !empty($bookDetail['gambar_buku']) ? htmlspecialchars($bookDetail['gambar_buku']) : 'https://via.placeholder.com/280x400?text='.urlencode($bookDetail['judul_buku']) ?>" alt="<?= htmlspecialchars($bookDetail['judul_buku']) ?>">
            </div>

            <!-- Book Info -->
            <div class="book-info">
                <h1 class="book-title"><?= htmlspecialchars($bookDetail['judul_buku']) ?></h1>

                <!-- Book Meta Information -->
                <div class="book-meta">
                    <div class="meta-item">
                        <span class="meta-label">Penerbit</span>
                        <span class="meta-value"><?= htmlspecialchars($bookDetail['penerbit'] ?? 'Tidak tersedia') ?></span>
                    </div>
                    <div class="meta-item">
                        <span class="meta-label">Kategori</span>
                        <span class="meta-value"><?= htmlspecialchars($bookDetail['kategori'] ?? 'Tidak tersedia') ?></span>
                    </div>
                    <div class="meta-item">
                        <span class="meta-label">Tahun Terbit</span>
                        <span class="meta-value"><?= htmlspecialchars($bookDetail['tahun_terbit'] ?? '—') ?></span>
                    </div>
                    <div class="meta-item">
                        <span class="meta-label">Jumlah Halaman</span>
                        <span class="meta-value"><?= htmlspecialchars($bookDetail['jumlah_halaman'] ?? '—') ?> halaman</span>
                    </div>
                </div>
                ...
                <div class="sinopsis-section">
                    <h4>Sinopsis</h4>
                    <div class="sinopsis-text" id="sinopsisText">
                        <p><?= htmlspecialchars($bookDetail['sinopsis'] ?? 'Tidak ada sinopsis') ?></p>
                        <div class="sinopsis-fade"></div>
                    </div>
                    <button class="btn-view-more" id="btnViewMore" onclick="toggleSinopsis()" style="display: none;">
                        <span id="btnText">Lihat selengkapnya</span> <i class="bi bi-chevron-down"></i>
                    </button>
                </div>

                <!-- Stok Info -->
                <div style="background: #f0f4ff; padding: 15px; border-radius: 8px; margin-bottom: 20px;">
                    <div style="color: #667eea; font-size: 0.85rem; font-weight: 700; text-transform: uppercase; margin-bottom: 5px;">
                        <i class="bi bi-box2"></i> Stok Tersedia
                    </div>
                    <div style="font-size: 1.5rem; font-weight: 700; color: #333;">
                        <?= htmlspecialchars($bookDetail['jumlah_buku']) ?> <span style="font-size: 0.9rem; color: #666;">Eksemplar</span>
                    </div>
                </div>

                <!-- Action Buttons -->
                <div class="action-buttons">
                    <a href="view_Tampil_buku.php?id_buku=<?= $bookDetail['id_buku'] ?>" class="btn-borrow-detail">
    ...
    <div class="form-container">
        ...
        <form method="POST" action="">
            <div class="form-group">
                ...
                <input type="text" id="judul_buku" name="judul_buku" value="<?= htmlspecialchars($borrowBook['judul_buku']) ?>" readonly>
                <input type="hidden" name="id_buku" value="<?= $borrowBookId ?>">
            </div>
            ...
            <div class="form-group">
                <label for="nomor_identitas">Nomor Identitas (NIS/NIM) *</label>
                <input type="text" id="nomor_identitas" name="nomor_identitas" placeholder="Masukkan nomor identitas" required>
            </div>

            <div class="form-group">
                <label for="jumlah_pinjam">Jumlah Pinjam *</label>
                <input type="number" id="jumlah_pinjam" name="jumlah_pinjam" value="1" min="1" max="3" required>
            </div>
            ...
            <div class="form-group">
                ...
                <textarea id="catatan" name="catatan" placeholder="Masukkan catatan jika ada..."></textarea>
            </div>
            ...
            <div class="form-buttons">
                ...
                <a href="view_Tampil_buku.php" class="btn-cancel">
    ...
    <div class="container-fluid">
        ...
        <div class="page-header">
            <h2 class="mb-0">
                <i class="bi bi-book-half" style="margin-right: 10px; color: #667eea;"></i>Koleksi Buku Perpustakaan
            </h2>
        </div>
            ...
            <div style="display: flex; gap: 15px; flex-wrap: wrap;">
                <div style="flex: 2; min-width: 250px;">
                    <div class="input-group">
                        ...
                        <input type="text" id="searchInput" class="form-control" placeholder="Cari judul, penerbit, atau kategori...">
                    </div>
                </div>
        ...
        <!-- Books Container -->
        <div class="books-grid" id="booksContainer" style="margin-left: auto; margin-right: auto; max-width: 1400px; padding: 0 20px;">
            <?php if (!empty($booksList)) { 
                foreach ($booksList as $b) { 
                    $cover = (!empty($b['gambar_buku'])) ? htmlspecialchars($b['gambar_buku']) : 'https://via.placeholder.com/300x400?text='.urlencode($b['judul_buku']);
            ?>
                <div class="book-col">
                    <div class="card book-card">
                        <div class="position-relative">
                            <img src="<?= $cover ?>" alt="<?= htmlspecialchars($b['judul_buku']) ?>" class="card-img-top book-image">
                            <span class="badge bg-primary badge-stock">Stok: <?= htmlspecialchars($b['jumlah_buku']) ?></span>
                        </div>
                        <div class="card-body d-flex flex-column" style="flex-grow: 1; display: flex; flex-direction: column;">
                            <h6 class="card-title" style="color: #333; font-weight: 700; margin-bottom: 12px; line-height: 1.4; min-height: 45px;"><?= htmlspecialchars($b['judul_buku']) ?></h6>
                            <p class="card-text small mb-2" style="color: #333; font-weight: 500;">
                                <i class="bi bi-shop" style="color: #667eea;"></i> <?= htmlspecialchars($b['penerbit'] ?? 'Penerbit Tidak Diketahui') ?>
                            </p>
                            <p class="card-text small mb-2" style="color: #333; font-weight: 500;">
                                <i class="bi bi-tag" style="color: #667eea;"></i> <?= htmlspecialchars($b['kategori'] ?? 'Kategori Tidak Diketahui') ?>
                            </p>
                            <p class="card-text small mb-3" style="color: #333; font-weight: 500; flex-grow: 1;">
                                <strong style="color: #667eea;">Tahun:</strong> <?= htmlspecialchars($b['tahun_terbit'] ?? '—') ?>
                            </p>
                            <div style="display: flex; flex-direction: column; gap: 10px;">
                                <a href="view_Tampil_buku.php?id=<?= $b['id_buku'] ?>" class="btn btn-outline-primary">
                                    <i class="bi bi-eye"></i> Detail
                                </a>
                                <a href="view_Tampil_buku.php?id_buku=<?= $b['id_buku'] ?>" class="btn btn-primary">
            ...
            <?php } 
                ...
                <div class="col-12">
                    <div class="alert alert-info text-center py-5">
                        <i class="bi bi-book-half fs-1 d-block mb-2"></i>
                        <p class="mb-0">Tidak ada buku tersedia saat ini.</p>
                    </div>
                </div>
            <?php } ?>
        </div>
        ...
        <!-- Pagination -->
        <?php if ($totalPages > 1) { ?>
        <nav aria-label="Page navigation" style="max-width: 1400px; margin-left: auto; margin-right: auto; padding: 0 20px;">
            <ul class="pagination">
                <!-- Previous Button -->
                <?php if ($currentPage > 1) { ?>
                    <li class="page-item">
                        <a class="page-link" href="?halaman=<?= $currentPage - 1 ?>">
                ...
                <?php for ($page = $startPage; $page <= $endPage; $page++) { ?>
                    <li class="page-item <?= ($page === $currentPage) ? 'active' : '' ?>">
                        <a class="page-link" href="?halaman=<?= $page ?>"><?= $page ?></a>
                    </li>
                <?php } ?>
                ...
                <?php if ($endPage < $totalPages) { ?>
                    ...
                    <?php } ?>
                    <li class="page-item">
                        <a class="page-link" href="?halaman=<?= $totalPages ?>"><?= $totalPages ?></a>
                    </li>
                <?php } ?>
        ...
        <div style="text-align: center; color: #999; max-width: 1400px; margin-left: auto; margin-right: auto; padding: 0 20px; margin-top: 15px;">
            <small>Halaman <?= $currentPage ?> dari <?= $totalPages ?> | Total: <?= $totalBooks ?> buku</small>
        </div>
        <?php } ?>
    </div>
    <?php } ?>
